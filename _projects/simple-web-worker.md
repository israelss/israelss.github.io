...
SWorker.run(
  (arg1, arg2) => `SWorker run 2: ${arg1} ${arg2}`,
  ["Another", "function in other thread"]
)
...
worker
  .postMessage("func1", ["Ignored", "arguments"])
  .then(console.log) // logs 'Worker 1: Working on func1'
  .catch(console.error); // logs any possible error
...
worker
  .postMessage("func3", ["Working on func3"])
...
worker
  .postMessage("func4", ["Overwrited argument"])
...
```javascript
const initialActions = [{ message: "func1", func: () => "Working on func1" }];
